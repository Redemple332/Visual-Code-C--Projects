...

{
    ...
    public partial class Admin : Form
    {
        ...
        private void all_DataDesc()
        {
            int i = 1;
            string sql = "SELECT * FROM allss order by date desc";
            sql_cmd = new MySqlCommand(sql, clsMySQL.sql_con);
            MySqlDataReader rd = sql_cmd.ExecuteReader();
            allss.Items.Clear();
            while (rd.Read())
        ...
        private void all_Dataasc()
        {
            int i = 1;
            string sql = "SELECT * FROM allss order by date asc";
            sql_cmd = new MySqlCommand(sql, clsMySQL.sql_con);
            MySqlDataReader rd = sql_cmd.ExecuteReader();
            allss.Items.Clear();
            while (rd.Read())
 ...
 private void agency()
 {
     ...
     cmd.ExecuteNonQuery();
        ...
        private void deleteAllToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Are you sure you want to delete all records ", "Delete all record data", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                if (MessageBox.Show("Are you sure you want to delete all records?", "Delete all record data", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (MessageBox.Show("Are you sure you want to delete all records ?", "Delete all record data", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
        ...
        private void tsearch_KeyDown(object sender, KeyEventArgs e)
        {
             if (e.KeyCode == Keys.Enter)
            {
                ...
                if (tsearch.Text != "" || ltag.Text != "History")
                {
                    Searhistory(tsearch.Text);
                }
            }
        ...
        private void viewsum_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Up)
            {
                viee();
        ...
        private void viewByIDToolStripMenuItem_Click(object sender, EventArgs e)
        {
            all_Data();
        }

        private void viewByDESCToolStripMenuItem_Click(object sender, EventArgs e)
        {
            all_DataDesc();
        }

        private void viewByASDToolStripMenuItem_Click(object sender, EventArgs e)
        {
            all_Dataasc();
        }
